...
require_macos
require_tools launchctl python3 shasum curl
...
"$SCRIPT_DIR/setup-ollama.sh" "aifred:latest" "$DATA_PARENT/model/Modelfile"
"$SCRIPT_DIR/start-host.sh"
echo "AIFRED Beta installed from commit $(cat "$OUT_ROOT/commit.txt"). Local chat is ready; user settings were retained."
