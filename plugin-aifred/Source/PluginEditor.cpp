...
void AifredAudioProcessorEditor::paint(juce::Graphics& g) {
  ...
  g.saveState();
  const auto matrix=responsiveTransformMatrix({designScale_,designOrigin_.x,designOrigin_.y,
                                                kDesignWidth*designScale_,kDesignHeight*designScale_});
  g.addTransform(juce::AffineTransform(matrix[0],matrix[1],matrix[2],matrix[3],matrix[4],matrix[5]));
