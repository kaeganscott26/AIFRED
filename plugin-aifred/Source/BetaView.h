...
inline std::array<float,6> responsiveTransformMatrix(const ResponsiveCanvas& canvas) noexcept
{
    // Construct the mapping explicitly so painted design coordinates and child
    // component bounds share the exact same unscaled letterbox origin.
    return {canvas.scale,0.0f,canvas.x,0.0f,canvas.scale,canvas.y};
}
